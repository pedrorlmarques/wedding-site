<template>
	<div class="text-center prose mx-auto my-8">
		<h1>Partilha momentos com os noivos!</h1>
		<p>
			Para que o nosso dia seja memorável e para que o possamos recordar no
			futuro, partilhem connosco fotografias e vídeos que capturaram durante os
			vários momentos, não se inibam :) Gostávamos ainda de reforçar que podem
			tirar fotografias e filmar, respeitem apenas o trabalho dos fotógrafos e
			videógrafos.
		</p>
	</div>
	<form :actions="false" @submit.prevent="onSubmit">
		<div
			...
			<ul
				...
				class="flex flex-1 flex-wrap ml-12 mt-4">
				<li
			...
			<div
				class="sm:flex flex-row items-center justify-between ml-12 mr-4 mt-4">
				<div class="flex flex-col sm:flex-row items-center justify-between">
					...
					<label
						for="img-upload"
						class="w-full md:w-auto border border-primary-500 text-primary-500 hover:bg-primary-500 hover:text-white px-4 py-2 rounded-lg hover:cursor-pointer flex flex-row items-center">
						<span class="mr-2">
							<svg
								class="fill-current w-4 h-4 ml-auto pt-"
								xmlns="http://www.w3.org/2000/svg"
					...
					<label
						for="video-upload"
						class="mt-2 ml-0 md:mt-0 w-full md:w-auto border border-primary-500 text-primary-500 hover:bg-primary-500 hover:text-white px-4 py-2 rounded-lg hover:cursor-pointer flex flex-row items-center md:ml-4">
						<span class="mr-2">
							<svg
								xmlns="http://www.w3.org/2000/svg"
				...
				<div class="mt-4 w-full sm:mt-0 sm:w-auto">
					<button
						...
						type="submit"
						:disabled="
							!valid && (imageData.length === 0 || videoData.length === 0)
						"
						class="border border-secondary-500 text-secondary-500 hover:bg-secondary-500 hover:text-white px-4 py-2 rounded-lg"
						:class="{
							'opacity-50 cursor-not-allowed hover:bg-transparent hover:text-secondary-500 hover:cursor-not-allowed':
								!valid && (imageData.length === 0 || videoData.length === 0),
						}">
						Publicar
					</button>
	...
	<div class="my-4 w-full sm:mt-0 sm:w-auto" v-if="publications.length">
		<ul>
			<li
				...
				:key="publication.id"
				class="mt-4 mx-4 sm:mx-0 bg-white rounded-xl shadow-lg border border-gray-200 p-8 cursor-pointer">
				...
				<div class="flex justify-between mt-4 items-center">
					<button
						...
						@click="onLikeButtonClick(publication)"
						:disabled="isLoadingLike">
						<HeartSvg
							class="w-6 h-6 stroke-2"
							:class="
								likedPublication(publication)
									? 'text-red-700 fill-current'
									: 'stroke-black'
							" />
						<span class="ml-2">{{ publication.likes.length }}</span>
					</button>

					<div v-show="publication.user_id === userSession?.user.id">
						<button
							class="text-red-700 hover:text-white border border-red-700 hover:bg-red-800 focus:ring-4 focus:outline-none focus:ring-red-300 font-medium rounded-lg px-5 py-2.5 text-center mr-2 text-xs sm:text-sm"
...
<script setup lang="ts">
	...
	const valid = computed(() => userPostText.value.length > 0);

	const updateNewLikes = useDebounceFn(
		async (likes: string[], publication: IPublication) => {
			await updateLikes(likes, publication);
		},
		500
	);

	const onLikeButtonClick = async (publication: IPublication) => {
		isLoadingLike.value = true;
		const likes = toggleLikeOnPublication(publication);
		if (likes.length !== publication.likes.length) {
			await updateNewLikes(likes, publication);
		}
		isLoadingLike.value = false;
	};

	const likedPublication = (publication: IPublication) => {
		return publication.likes.includes(userSession.value?.user.id!);
	};

	const toggleLikeOnPublication = (publication: IPublication): string[] => {
		let likes: string[] = [];

		if (likedPublication(publication)) {
			likes = publication.likes.filter(
				(id) => id !== userSession.value?.user.id!
			);
		} else {
			likes = [...publication.likes, userSession.value?.user.id!];
		}

		return likes;
	};

	const updateLikes = async (likes: string[], publication: IPublication) => {
		try {
			...
			toast.error('Aconteceu um erro, reportar ao Administrador');
		}
		...
		const uploadFileToBucket = await Promise.all(
			publicationData.value.map(async ({ file, name }) => {
				...
				if (!error) {
					...
					return {
						path: fileData?.signedUrl as string,
						type: file.type,
						name: `${now}_${userSession.value?.user.id}_${fileName}`,
					};
				}
			})
		);
	...
	const deletePublication = async (publication: IPublication) => {
		if (publication.files.length) {
			const filesToRemove = publication.files.map((file) => file.name);
